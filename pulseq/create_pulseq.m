...
if params.gen.seq == 3
    tr_tmp = 3.5e-3+mr.calcDuration(gz_blips(1))+mr.calcDuration(gx)+mr.calcDuration(gx_spoil)+params.gen.tr_delay; % aprox TR
    tr_tmp = tr_tmp + max(params.gen.multi_te);
else
...
%     params_me_gre.gen.n(3) = round(params_me_gre.gen.n(3)/params_me_gre.gen.kz/params_me_gre.gen.pf/2)*2;
    ...
    if round((params_me_gre.gen.fov(3)/params_me_gre.gen.kz)*1000,9)/round((params_me_gre.gen.fov(3)/params_me_gre.gen.kz)*1000) ~= 1
        params_me_gre.gen.fov(3) = round(params_me_gre.gen.fov(3)*1000/2/params_me_gre.gen.kz)*2*params_me_gre.gen.kz*1e-3;
    end
...
%     sk_int_delay    = mr.makeDelay(400e-3);  % original
    ...
    sk_no_rf_delay = mr.makeDelay(mr.calcDuration(rf0(1)));
    sk_no_fs_rf_delay = mr.makeDelay(mr.calcDuration(rf_fs));
...
%                 if params.gen.fid_nav == 1; te_delay.delay = te_delay.delay-(fid_nav_adc.duration*2)-mr.calcDuration(fid_nav_delay1)-mr.calcDuration(fid_nav_delay2); end
    ...
    for i=1:params.gen.echos-1
        fm_te_delay1(i) = mr.makeDelay(round(max(fm_te_delay0.delay)-fm_te_delay0(i).delay,4));
    end
        ...
        % for m=1:params.gen.n(3)
            for j = 1:last_seg
                ...
                if (params.gen.fat_sat && params.gen.fs_interl==1) || (params.gen.fat_sat && params.gen.fs_interl==0)
                    seq_sk.addBlock(gx_fs,gy_fs,gz_fs);    % spoilers
                    seq_sk.addBlock(no_fs_delay); 
                end
                % Spoil
...
%                         end
                        ...
                        % if params.gen.tr_delay > 0; seq.addBlock(tr_delay); end % TR delay
                        if (params.gen.seq == 3 && params.gen.multi_te(1) > 0 && l < params.gen.echos)
                            seq.addBlock(fm_te_delay1(l));
                        end
