...
# Some parameters
params[:scan] = "sb_004_SS_SO_2rz_full_brain"            # For now: if multipe echos, include _e1.. _e2..
params[:scan_b0] = "sb_001_2rz_full_brain"           # Name of the ME-GRE to use for CS and B0map, ONLY USED WHEN SEPARATE FIELDMAP SCAN...
params[:directory] = "11132024_sb_girf"        # directory where the data is stored
...
if params_pulseq["gen"]["me_gre"] > 0 && params[:isPhantom] == false
    ...
    file_name = string(string(params[:scan_b0][1],params[:scan_b0][idx]),"_",Int(params_pulseq["gen"]["n_ov"][1]),"_",Int(params_pulseq["gen"]["n_ov"][2]),"_",Int(params_pulseq["gen"]["n_ov"][3]))
end
...
if input == "n"
    ...
    niwrite(string(params[:path],"/tmp/",params[:scan],"_",params[:traj_type],".nii"),NIVolume(b0_nii; voxel_size=Tuple(params_pulseq["gen"]["res"].*1e3)))
end
...
if params[:do_pi_recon]
    ...
    if input == "n"
        ...
        SensitivityMap = CalculateSensitivityMap(recon_b0,Tuple(Int.(params_pulseq["gen"]["n"])))
...
params_recon[:reconSize] = (params_pulseq["gen"]["n"][1],params_pulseq["gen"]["n"][2],params_pulseq["gen"]["n"][3])
...
if params[:do_rDORK_corr]
    ...
    nav_ref, ndata = FormatRawData(rawData,params;single_rep=true,rep_format=RepRef,fid_nav=Int(params_pulseq["gen"]["fid_nav"]))

    if Int(params_pulseq["gen"]["fid_nav"]) == 1
        nav_ref = deepcopy(ndata[:,:,:,1,1])   # Taking only the first FID
        nav_range = 6:50
    else 
        if params_pulseq["spi"]["type"] == 0
            nav_range = 6:20
    ...
    if params_pulseq["gen"]["seq"] == 1
        ...
        nav_ref1, ndata1 = FormatRawData(rawData,params;single_rep=true,rep_format=RepRef,fid_nav=Int(params_pulseq["gen"]["fid_nav"]))
        if Int(params_pulseq["gen"]["fid_nav"]) == 1
            nav_ref1 = deepcopy(ndata1[:,:,:,1,1])   # Taking only the first FID
        end
        if params[:kz_enc] == 0  # Linear
...
@time begin
    for i_rep=params[:rep_recon]
        ...
        if params[:do_rDORK_corr]
            if Int(params_pulseq["gen"]["fid_nav"]) == 1
                nav = mean(ndata[nav_range,Int(params_pulseq["gen"]["n_ov"][3]/2)+1,:,1,1],dims=2)      # Taking just first FID
            else
                nav = nothing
            end
            if params_pulseq["gen"]["seq"] == 1  # VASO and BOLD separately
                if isodd(i_rep)  # VASO
                    tmp = CorrectRepetitionDORK(tmp,nav_ref,nav_range,params,nav)
                else            # BOLD
                    tmp = CorrectRepetitionDORK(tmp,nav_ref1,nav_range,params,nav)
                end
            else
                tmp = CorrectRepetitionDORK(tmp,nav_ref,nav_range,params,nav)
            end
    ...
    for i_contrasts = 1:length(params[:contrasts])
        ...
        TimeSeries = MergeReconstrucion(params[:path],params[:scan],tmp_rep_recon,params[:do_pi_recon],params[:do_b0_corr],
                    params[:do_coco_corr],params[:do_k0_corr],params[:do_rDORK_corr],params[:do_high_b0_corr]; 
