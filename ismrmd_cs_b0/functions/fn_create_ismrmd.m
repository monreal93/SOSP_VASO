...
function fn_create_ismrmd(folder,scan,params)
    ...
    if contains(scan,'sv')
        if params.is2d == 1
            filename = sprintf('./data/%s/ismrmd/%s_v_r1_2d.h5',folder,scan);
        else
            filename = sprintf('./data/%s/ismrmd/%s_v_r1_3d.h5',folder,scan);
        end
    elseif contains(scan,'abc')
        if params.is2d == 1
            filename = sprintf('./data/%s/ismrmd/%s_r1_2d.h5',folder,scan);
        else
            filename = sprintf('./data/%s/ismrmd/%s_r1_3d.h5',folder,scan);
        end
    end
    tmp = 'y';
    if exist(filename) > 1
